...
INSERT INTO NhanVien(HoTen, VaiTro, GioiTinh, DiaChi, SDT, Luong, MatKhau) 
VALUES
(N'Tran Tuan Hung', 0, 1, N'Phạm Văn Đồng, Ngo Gia, Hồ Chí Minh', '0367864937', 1100, '0520'),
(N'Phạm Van Nam', 0, 1, N'Phạm Văn Đồng, Bình Tan, Hồ Chí Minh', '0367864937', 1100, '0320');
...
INSERT INTO KhachHang (HoTen, CCCD, SDT, DiaChi, GioiTinh)
VALUES 
(N'Trần Nam Y', '123456789012', '0912345678', N'789 Đường Ly Thai To, Quận 3, TP. HCM', 1)
go
...
SELECT * FROM PhieuDatPhong ORDER BY NgayDatPhong DESC

select * from PhieuDatPhong
use KhachSan
SELECT * from PhieuDatPhong WHERE MaKH = 'KH010' AND NgayDatPhong BETWEEN DATEADD(MINUTE, -20, GETDATE()) AND DATEADD(MINUTE, 20, GETDATE())

SELECT MaPDP FROM PhieuDatPhong WHERE MaKH = ? AND NgayDatPhong BETWEEN DATEADD(MINUTE, -20, ?) AND DATEADD(MINUTE, 20, ?);
...
SELECT 
	...
	dv.DonGia, 
	ct.SoLanSuDung, 
	(dv.DonGia*ct.SoLanSuDung) as 'Thanh Tien' 
FROM PhieuDichVu p 
	inner join ChiTietPhieuDichVu ct on p.MaPDV = ct.MaPDV 
	inner join DichVu dv on ct.MaDV = dv.MaDV
where MaPhong = 'P001' AND MaPDP = 'PDP0001'
use KhachSan
SELECT * FROM ChiTietPhieuDatPhong WHERE MaPDP = 'PDP0001' AND TinhTrang = N'Thanh toán'
SELECT * FROM ChiTietPhieuDatPhong WHERE MaPDP = 'PDP0002' AND TinhTrang = N'Chưa Thanh toán'

SELECT ct.MaPhong, p.DonGia, ct.NgayNhanPhong, ct.NgayTraPhong, DATEDIFF(DAY,ct.NgayNhanPhong,ct.NgayTraPhong) * p.DonGia as 'Thanh Tien' FROM ChiTietPhieuDatPhong ct INNER JOIN Phong p on ct.MaPhong = p.MaPhong WHERE MaPDP = 'PDP0002' AND ct.MaPhong = 'P002'
...
exec sp_ChiTietPhieuDatPhongDa1 'P001', 'PDP0001'
...
SELECT 
			...
			ct.NgayTraPhong, 
			DATEDIFF(DAY,ct.NgayNhanPhong,ct.NgayTraPhong) * p.DonGia as 'ThanhTien' 
		FROM KhachHang kh INNER JOIN PhieuDatPhong pd ON kh.MaKH = pd.MaKH INNER JOIN ChiTietPhieuDatPhong ct ON pd.MaPDP = ct.MaPDP  
			INNER JOIN Phong p on ct.MaPhong = p.MaPhong 
		WHERE MaPDP = 'PDP0001' AND ct.MaPhong = 'P001'


go
CREATE PROC sp_ChiTietPhieuDich(@maPhong NVARCHAR(4), @maPhieuDatPhong NVARCHAR(7))
AS BEGIN
		SELECT 
			dv.TenDV, 
			dv.DonGia, 
			ct.SoLanSuDung, 
			(dv.DonGia*ct.SoLanSuDung) ThanhTien
		FROM PhieuDichVu p 
			inner join ChiTietPhieuDichVu ct on p.MaPDV = ct.MaPDV 
			inner join DichVu dv on ct.MaDV = dv.MaDV
		where MaPhong = @maPhong AND MaPDP = @maPhieuDatPhong
END;
...
exec sp_ChiTietPhieuDichVu 'P001', 'PDP0001'



SELECT MaPDV FROM PhieuDichVu where MaPhong = 'P001' AND MaPDP = 'PDP0001'
SELECT * FROM ChiTietPhieuDichVu WHERE MaPDV = 1
-- Thêm dữ liệu vào bảng PhieuHoaDon
-- Dữ liệu cho bảng PhieuHoaDon
-- Mỗi phiếu đặt phòng (MaPDP) có thể đặt nhiều phòng (MaPhong)
-- Mã nhân viên (MaNV) được liên kết với các phiếu đặt phòng để theo dõi người lập phiếu
